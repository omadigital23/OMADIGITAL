    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ? 'bg-bg-primary/85 backdrop-blur-xl border-b border-border-subtle shadow-lg'
            : 'bg-transparent'
        }`}
      >
        {/* Scroll progress bar */}
        <div
          className="absolute bottom-0 left-0 h-[2px] bg-gradient-to-r from-accent-blue via-accent-violet to-accent-purple transition-all duration-100"
        ...
        <div className="container-custom flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden md:flex items-center gap-4">
            <button
              ...
              aria-label={`Changer de langue — ${locale === 'fr' ? 'English' : 'Français'}`}
            >
              {t('languageSwitch')}
            </button>
            <a
              ...
              className="gradient-bg text-white text-sm font-medium px-5 py-2.5 rounded-full hover:shadow-glow transition-all hover:scale-105 flex items-center gap-2"
            >
          ...
          <button
            ...
            className="md:hidden flex flex-col gap-1.5 p-2 rounded-lg hover:bg-bg-glass transition-colors"

// Composant principal du chatbot amélioré
...
export function SmartChatbotNext() {
  ...
  // Initialiser d'abord les hooks de base
  ...
  // Enhanced TTS request handling with error recovery
  const handleTTSRequest = React.useCallback((text: string, language: 'fr' | 'en', inputMethod: 'text' | 'voice') => {
    ...
    // TTS automatique pour les inputs vocaux, même si désactivé par défaut
    // Note: We now use proxy-only TTS for better security
    if ((ttsState.isActive || inputMethod === 'voice') && ttsState.isSupported) {
      try {
        speakText(text, language);
      } catch (ttsError) {
        console.error('SmartChatbotNext: TTS error:', ttsError);
        // Don't show error to user for TTS failures - it's not critical
      }
    } else {
      console.log('SmartChatbotNext: TTS skipped:', { 
  ...
  const handleVoiceTranscript = React.useCallback(async (transcript: string) => {
    console.log('SmartChatbotNext: 🚀 ENVOI DIRECT - Transcription:', transcript);
    ...
    while (retryCount <= maxRetries) {
      try {
        // Envoyer directement sans toucher à l'input
        await sendMessage(transcript.trim(), 'voice');
        console.log('SmartChatbotNext: ✅ Message vocal envoyé directement');
        break;
      } catch (err) {
        retryCount++;
        console.error(`SmartChatbotNext: ❌ Erreur envoi message vocal (tentative ${retryCount}):`, err);

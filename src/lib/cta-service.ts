...
class CTAService {
  ...
  async getActiveCTAs(language?: 'fr' | 'en'): Promise<CTAAction[]> {
    ...
    try {
      ...
      if (language) {
        // The cta_actions table has a conditions column which is a JSON field
        // We need to filter by the language in the conditions JSON
        try {
          query = query.or(`conditions->>language.eq.${language},conditions->>language.eq.both`);
        } catch (filterError) {
          console.warn('Language filter failed, returning all active CTAs:', filterError);
          // If language filtering fails, just return all active CTAs
        }
      }
  ...
  async findBestCTA(message: string, language: 'fr' | 'en'): Promise<CTAAction | null> {
    ...
    return scoredCTAs.length > 0 && scoredCTAs[0] ? scoredCTAs[0].cta : null;
  }

...
import { AdminChatbotProps } from '../types/admin';
import { ChatbotInteraction, LoadingState } from '../types/analytics';
...
export function AdminChatbotSessions({ onViewDetails }: AdminChatbotSessionsProps) {
  ...
  const translateStatus = (status: string): string => {
    const key = `chatbot.sessionstable.status.${status}`;
    return t(key) === key ? status : t(key);
  };

  const translateType = (type: string): string => {
    const key = `chatbot.sessionstable.type.${type}`;
    ...
    return (
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="flex justify-center items-center h-64">
          ...
          <span className="ml-4 text-gray-600">{t('chatbot.sessions.loading')}</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('chatbot.sessions.error.title')}</h3>
          <p className="text-gray-600 mb-4">
            {t('chatbot.sessions.error.message')}
          </p>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            ...
            {t('chatbot.sessions.title')}
          </h2>
          <p className="text-gray-600 mt-1">
            {t('chatbot.sessions.description')}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder={t('chatbot.sessions.search.placeholder')}
              value={searchTerm}
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <option value="all">{t('chatbot.sessions.filters.status.all')}</option>
            ...
            <select
              ...
              className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500"
            >
              <option value="all">{t('chatbot.sessions.filters.type.all')}</option>
            ...
            <select
              ...
              className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500"
            >
              <option value="date">{t('chatbot.sessions.sort.date')}</option>
              <option value="messages">{t('chatbot.sessions.sort.messages')}</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="text-left py-4 px-6 font-medium text-gray-700">{t('chatbot.sessions.table.user')}</th>
                <th className="text-left py-4 px-6 font-medium text-gray-700">{t('chatbot.sessions.table.date')}</th>
                <th className="text-left py-4 px-6 font-medium text-gray-700">{t('chatbot.sessions.table.status')}</th>
                <th className="text-left py-4 px-6 font-medium text-gray-700">{t('chatbot.sessions.table.type')}</th>
                <th className="text-left py-4 px-6 font-medium text-gray-700">{t('chatbot.sessions.table.messages')}</th>
                <th className="text-left py-4 px-6 font-medium text-gray-700">{t('chatbot.sessions.table.lastMessage')}</th>
                <th className="text-left py-4 px-6 font-medium text-gray-700">{t('chatbot.sessions.table.actions')}</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={session.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-4 px-6">
                    ...
                    <div className="text-sm text-gray-500 flex items-center gap-1 mt-1">
                      ...
                      <span>{new Date(session.date).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                  </td>
                  <td className="py-4 px-6">
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-2">
                      <button 
                        ...
                        title={t('chatbot.sessions.table.actions.view') || "Voir les détails"}
                        onClick={() => onViewDetails && onViewDetails(session.id)}
                      >
                        <Eye className="w-4 h-4" />
                      </button>
                      <button className="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors"
                        title={t('chatbot.sessions.table.actions.more') || "Plus d'options"}>
        ...
        {filteredSessions.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('chatbot.sessions.noResults.title')}</h3>
            <p className="text-gray-600">
              {t('chatbot.sessions.noResults')}
            </p>
          </div>
        )}
      </div>

...
export default requireAdminApi(async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Note: Fonctions RPC additionnelles désactivées temporairement
    // Elles seront réactivées une fois les migrations appliquées
    const convStats = null;
    const interactionStats = null;
    const quoteStats = null;
    const blogStats = null;
    const ctaStats = null;
    ...
    const activities: Array<{id: string, type: string, description: string, timestamp: string, status: 'success' | 'error' | 'warning'}> = [];

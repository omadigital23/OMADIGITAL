...
async function testAdminApiDirect() {
  console.log('🔍 Testing admin API logic directly...');
  ...
  try {
    ...
    // Simulate the exact queries from the dashboard-metrics API
    console.log('\n📡 Executing dashboard queries...');
    
    const periodDays = 7;
    const startDate = new Date(Date.now() - periodDays * 24 * 60 * 60 * 1000).toISOString();

    // Execute all queries in parallel like the API does
    const [
      interactionsResult,
      quotesResult,
      subscribersResult,
    ...
    const recentSessions = interactions.filter(i => 
      new Date(i.created_at).getTime() > Date.now() - 2 * 60 * 60 * 1000
    );

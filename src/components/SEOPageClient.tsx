...
export default function SEOPageClient({
  title, subtitle, description, localContext, faqs, serviceEmoji, pricingInfo,
}: SEOPageProps) {
  ...
  return (
    <>
      ...
      <main className="pt-24 pb-20">
        ...
        {/* Pricing Preview */}
        <section className="container-custom mb-20">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="max-w-3xl mx-auto text-center">
            <div className="p-8 rounded-2xl bg-gradient-to-br from-accent-violet/10 to-accent-blue/10 border border-accent-violet/20">
              <h2 className="font-heading font-bold text-2xl mb-4">Nos tarifs</h2>
              <p className="text-text-secondary mb-2">À partir de</p>
              <p className="font-heading font-bold text-4xl gradient-text mb-1">{pricingInfo.from} FCFA</p>
              <p className="text-text-muted text-sm mb-6">Jusqu&apos;à {pricingInfo.to} FCFA pour les solutions premium</p>
          ...
          <section className="container-custom mb-20">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="max-w-3xl mx-auto">
              ...
              <div className="space-y-4">
                {faqs.map((faq, i) => (
                  <details key={i} className="group p-6 rounded-xl bg-bg-card border border-border-subtle">
                    <summary className="font-heading font-medium text-text-primary cursor-pointer list-none flex items-center justify-between">
                      {faq.q}
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="shrink-0 group-open:rotate-180 transition-transform">
                        <path d="M6 9l6 6 6-6" />
                      </svg>
                    </summary>
        ...
        <section className="container-custom">
          <div className="max-w-3xl mx-auto text-center p-12 rounded-2xl gradient-bg">
            <h2 className="font-heading font-bold text-2xl md:text-3xl text-white mb-4">{t('ctaText')}</h2>
            <p className="text-white/70 mb-6">{t('ctaSubtext')}</p>
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-white text-bg-primary font-medium px-8 py-3 rounded-full hover:shadow-lg transition-all">
              WhatsApp →
            </a>
          </div>
        </section>

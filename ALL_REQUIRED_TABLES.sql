-- ALL REQUIRED TABLES FOR OMA DIGITAL PROJECT (CLEAN VERSION)
-- This file contains all the SQL statements to create the required tables for your Supabase database without blog tables
...
-- Chatbot interactions RLS policies
...
-- Performance monitoring RLS policies
CREATE POLICY "Allow inserts for performance data" ON performance_alerts 
  FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow selects for admins" ON performance_alerts 
  FOR SELECT USING (COALESCE(auth.jwt() ->> 'role', '') = 'admin');

CREATE POLICY "Allow inserts for performance data" ON pagespeed_results 
  FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow selects for admins" ON pagespeed_results 
  FOR SELECT USING (COALESCE(auth.jwt() ->> 'role', '') = 'admin');

CREATE POLICY "Allow inserts for performance data" ON performance_budget_checks 
  FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow selects for admins" ON performance_budget_checks 
  FOR SELECT USING (COALESCE(auth.jwt() ->> 'role', '') = 'admin');

CREATE POLICY "Allow inserts for performance data" ON performance_trends 
  FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow selects for admins" ON performance_trends 
  FOR SELECT USING (COALESCE(auth.jwt() ->> 'role', '') = 'admin');

CREATE POLICY "Allow inserts for performance data" ON real_user_metrics 
  FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow selects for admins" ON real_user_metrics 
  FOR SELECT USING (COALESCE(auth.jwt() ->> 'role', '') = 'admin');
...
GRANT INSERT ON performance_alerts TO anon;
GRANT SELECT ON performance_alerts TO authenticated;

GRANT INSERT ON pagespeed_results TO anon;
GRANT SELECT ON pagespeed_results TO authenticated;

GRANT INSERT ON performance_budget_checks TO anon;
GRANT SELECT ON performance_budget_checks TO authenticated;

GRANT INSERT ON performance_trends TO anon;
GRANT SELECT ON performance_trends TO authenticated;

GRANT INSERT ON real_user_metrics TO anon;
GRANT SELECT ON real_user_metrics TO authenticated;

-- Targeted fix for blog_subscribers table only
-- This avoids dropping functions that are used by other tables

-- First, let's drop only the triggers on blog_subscribers table
...
-- Create a separate function for blog_subscribers updated_at column
CREATE OR REPLACE FUNCTION update_blog_subscribers_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
-- Only fix the variable naming issue in this specific function
...
-- Fix the log_newsletter_event function with explicit variable names
-- This is the main source of the subscriber_id ambiguity error
...
DECLARE
    ...
    v_analytics_id UUID;
BEGIN
    -- Find the subscriber by email (explicit table reference)
    SELECT bs.id INTO v_subscriber_id 
    FROM public.blog_subscribers bs
    WHERE bs.email = p_subscriber_email;
...
-- Recreate only the triggers for blog_subscribers table
